...
fail('49','You end up shooting the suspect...','you hear a groan and a thud from inside the portal and then walk in. You shot the suspect in the head by accident. You failed','restart','restart','retry','scene1','scene1','scene43');
...
fail('18','','Unfortunately, you used it the WRONG way. You failed','start over','retry','restart','scene1','scene16','scene1')
...
//deaths
{
fail('50','The suspect shoots you','Bam! A straight shot through the eye kills you before you even knew you were in there.','X','restart','retry','scene1','scene1','scene43');
fail('45','That was a false eye!','That, my friend (or enemy) was a light. The real sensor was on the body. It pulls the trigger, blasting the bullet through your head.','restart','start over','random checkpoint','scene1','scene1','scene41');
fail('44','The bullet bounces off the robot\'s hand and it pulls the trigger. You died (with a huge hole in your head).','X','restart','retry','scene1','scene1','scene41');
fail('42','The robot looks as though it is melting...','...and it slowly drips to the floor. Just before its legs completely melt, it shoots you straight through the neck, killing you.','start over','retry','restart','scene1','scene40','scene1');
fail('37','You died!','you apparently didn\'t read properly! the cyborg gets out a tazor and zaps you dead.','start over','retry','revive','scene1','scene30','scene1');
...
fail('29','Too much explosive power',' Sadly, this place is an abandoned nuclear factory, which is not the stablest thing in the world... so... the place cracks under the explosive power and you fall into the hole and get blown up by millions of pounds of uranium.','X','start over','restart','scene1','scene25','scene1');
fail('28','Beware of turret!','A turret, camoflaged, emerges from the wall and blasts the life out of you with a single hit.','start over','retry','restart','scene1','scene25','scene1');
fail('26','Too much explosive power!','First you fire the rocket into the hall, killing all the enemies that you see, then you throw the grenade. Sadly, this place is an abandoned nuclear factory, which is not the stablest thing in the world... so... the place cracks under the explosive power and you fall into the hole and get blown up by millions of pounds of uranium.','start over','restart','retry','scene1','scene1','scene22')
fail('24','Not enough power!','The grenade blows up a few robots, but not all. A few thousand laser bullets cut through you. You died','start over','retry','restart','scene1','scene22','scene1')
fail('23','Boooom!','Unfortunately, you forgot that this was an abandoned nuclear factory. You ignite a few hundred thousand tons of uranium and the whole place explodes.','restart','retry','start over','scene1','scene20','scene1');
fail('21','you died!','You walk carelessly into the hall, and what seems like a million bullets fly through you.','restart','start over','retry','scene1','scene1','scene20');
